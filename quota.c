...
int xfs_list_quotas_simple(const char *path, int type, XFSQuotaList *list, int max_id) {
    if (!path || !list) {
        return EINVAL;
    }
    ...
    for (uint32_t id = 0; id < (uint32_t)list->capacity && found < list->capacity; id++) {
        ...
        int ret = quotactl(QCMD(Q_XGETQUOTA, type), path, id, (caddr_t)&dq);
        ...
        info->id = id;
        info->qtype = type;
        info->bhardlimit = dq.d_blk_hardlimit;
        info->bsoftlimit = dq.d_blk_softlimit;
        info->curblocks = dq.d_bcount;
        info->ihardlimit = dq.d_ino_hardlimit;
        info->isoftlimit = dq.d_ino_softlimit;
        info->curinodes = dq.d_icount;
        info->btime = dq.d_btimer;
